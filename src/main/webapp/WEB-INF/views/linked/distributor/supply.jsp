...
<div class="wrapper">
    ...
    <table>
        ...
        <c:if test="${!empty supplyList}">
            <c:forEach items="${supplyList}" var="supply">
                <tr>
                    ...
                    <td>${supply.price}</td>
                    <td>
                        <a href="<c:url value="/linked/distributor/${distributor.ID}/delete/${supply.ID}"/>">
                            <img src="<c:url value="/resources/images/remove-icon.png"/>"/>
                        </a>
                        <a href="<c:url value="/linked/distributor/${distributor.ID}/update/supply?s_id=${supply.ID}"/>">
        ...
        <tr>
            <c:url value="/linked/distributor/${distributor.ID}/save-or-update" var="saveOrUpdateSupply"/>
            <form:form method="post" action="${saveOrUpdateSupply}" modelAttribute="supplyDTO">
                <td><form:input path="ID" readonly="true"/></td>
                <td>
                    <form:select path="distributorName">
                        <form:option value="${distributor.name}"/>
                    </form:select>
                </td>
                <td>
                ...
                <td>
                    <button type="submit">
                        <c:if test="${supplyDTO.ID == null}">Add</c:if>
                        <c:if test="${supplyDTO.ID != null}">Update</c:if>
                    </button>
                </td>
            </form:form>
        </tr>
